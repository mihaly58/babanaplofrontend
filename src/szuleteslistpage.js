...
export function SzuletesListPage() {
    ...
    const filteredSzuletesek = szuletesek.filter(szuletesek => szuletesek.felhasznaloId === localStorage.getItem("userid"));
        ...
        <div>
            ...
            {isFetchPending ? (
                ...
                <div className='m-auto p-5 text-center content bg-lavender'>
                    <h2 className='label-container'>Születések</h2>
                    <NavLink to={'/uj-szuletes'} className={({isActive}) => "nav-link" + (isActive ? "active" : "")}>
                    <button type="button" class="btn btn-info">Új születés</button>
                    </NavLink>
                    {filteredSzuletesek.map((szuletes) => (
